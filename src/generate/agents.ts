...
function usingWithAiFile(plan: VaultPlan): GeneratedFile {
  ...
  const content = joinSections([
    ...
    [
      "## MCP — the vault as tools instead of files",
      "",
      `Skills tell an agent which command to run. MCP goes one step further: \`vulcanus serve\` exposes ${vault.name} as typed tools, so recall is a call with a structured answer rather than a file read the agent has to parse.`,
      "",
      "Register it once with an MCP client:",
      "",
      "```bash",
      "claude mcp add vulcanus -- vulcanus serve",
      "```",
      "",
      "The command runs inside the vault, or in any directory beneath it — the vault root is resolved by walking up to `vulcanus.json`. Any MCP client works the same way; the transport is stdio.",
      "",
      "The tools it exposes:",
      "",
      bulletList([
        "`recall` — a project's Capsule plus the read-next order. The entry point before working on anything.",
        "`search` — layer-aware text search; Capsule and Recall Map hits rank first.",
        "`list_projects` — the routing table: names, triggers, status, capsule paths.",
        "`append_decision` — record a confirmed decision in the project's Decisions note.",
        "`append_rule` — record a standing rule in the project's Rules note.",
        "`update_capsule` — refresh one section of a Capsule after something it summarizes changed.",
        "`vault_status` and `doctor` — health, stale capsules, and full validation.",
      ]),
      "",
      "The writing tools are for confirmed outcomes only. `recall` says so itself: when a Capsule is older than the Decisions or Rules beneath it, the answer carries a staleness warning instead of pretending the summary is current.",
    ].join("\n"),
    [
      "## Keeping it honest",
      "",
      bulletList([
        ...
        "`vulcanus stats` — what a cold-start agent must read, and what task-scoped recall saves.",
        "`vulcanus serve` — expose the vault to MCP clients.",
      ]),
      "",
